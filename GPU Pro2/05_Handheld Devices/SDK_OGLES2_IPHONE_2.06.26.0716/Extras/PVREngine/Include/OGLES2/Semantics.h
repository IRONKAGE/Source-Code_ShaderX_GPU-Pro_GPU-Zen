...
namespace pvrengine
{
	...
	const static EUniformSemantic eFrameUniforms[i32NUMFRAMEUNIFORMS+1] =
	{
		eUsVIEW,
			...
			eUsLIGHTPOSWORLD,
			eUsLIGHTPOSEYE,
			eUsLIGHTDIRWORLD,	// can also be mesh uniform if light type doesn't match
			eUsLIGHTDIREYE,	// can also be mesh uniform if light type doesn't match
			eUsEYEPOSWORLD,
			eUsANIMATION,
			eUsVIEWPORTPIXELSIZE,
			eUsVIEWPORTCLIPPING,
	...
	const static EUniformSemantic eMeshUniforms[] =
	{
		eUsPosition,
			...
			eUsLIGHTPOSMODEL,
			eUsLIGHTDIRMODEL,
			eUsLIGHTDIRWORLD,	// can also be frame uniform if light type does match
			eUsLIGHTDIREYE,	// can also be frame uniform if light type does match
			eUsEYEPOSWORLD,
			eUsGEOMETRYCOUNTER,
			eUsBOUNDINGCENTER,
			eUsBOUNDINGSPHERERADIUS,
