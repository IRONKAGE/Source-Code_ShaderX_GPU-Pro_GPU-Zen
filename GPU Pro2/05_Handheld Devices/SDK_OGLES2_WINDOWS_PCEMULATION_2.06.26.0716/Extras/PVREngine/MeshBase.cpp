...
namespace pvrengine
{
	...
	Mesh::Mesh(CPVRTModelPOD* psScene, const int iIndex, MaterialManager *pMaterialManager)
	{
		Init();
		m_psScene = psScene;
		m_psNode = &m_psScene->pNode[iIndex];
		m_psMesh = &m_psScene->pMesh[m_psNode->nIdx];

		m_pMaterial = pMaterialManager->getMaterial(m_psNode->nIdxMaterial);
	...
	bool Mesh::setDrawMode(const EDrawMode eDrawMode)
	{
...
/******************************************************************************/
	...
	CPVRTString Mesh::getDrawModeName(const EDrawMode eDrawMode) const
	{
		return g_strDrawModeNames[eDrawMode];
	}
