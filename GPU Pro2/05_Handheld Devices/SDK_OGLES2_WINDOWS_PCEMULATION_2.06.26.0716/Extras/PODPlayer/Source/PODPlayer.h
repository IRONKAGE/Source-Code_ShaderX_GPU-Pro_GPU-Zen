...
protected:
	...
	// Camera
	pvrengine::SimpleCamera	m_Camera;
	...
	pvrengine::MeshManager		m_MeshManager, m_TransparentMeshManager;
	static const int kiOpaque = 0;

	// Light Manager
	pvrengine::LightManager	m_LightManager;

	// Material Manager
	pvrengine::MaterialManager	m_MaterialManager;

	// camera number to use from scene
	unsigned int m_u32CurrentCameraNum;

	// string for onscreen display.
	CPVRTString				m_strPrint3DString;

	// string for using as title
	CPVRTString		m_strDemoTitle;

	// used to determine the intialisation state
	int				m_i32Initialising, m_i32InitSub;	
	...
	// Uniforms and attributes manager
	pvrengine::UniformHandler	m_UniformHandler;

	// time functions manager
