...
bool OGLES2ShadowVolumes::RenderScene()
{
	...
	int i32Time = PVRShellGetTime();
	int i32DeltaTime = i32Time - m_i32TimePrev;

	m_i32TimePrev = i32Time;

    // If the cog is classed as dynamic then we need to update its angle of rotation
	if(m_i32ObjectType[eBigCog] == eDynamicObject)
	{
		m_fBigCogAngle   += i32DeltaTime * 0.001f;
	...
	{
		m_fSmallCogAngle -= i32DeltaTime * 0.004f;
	...
	*/
	...
	m_Print3D.DisplayDefaultTitle("ShadowVolumes", "", ePVRTPrint3DLogoIMG);
	m_Print3D.Flush();
