/******************************************************************************
 ...
 @Description  Shows how to do simple motion blur
...
class OGLES2Bloom : public PVRShell
{
	...
	float m_fModelAngleY;
	float m_fLightAngleY;
...
bool OGLES2Bloom::LoadTextures(CPVRTString* pErrorStr)
{
	...
	*/
	...
	/*
	    Check if the depth texture extension is supported. If not, the initial render to texture pass
	    may contain incorrect results when objects are overlapping or concave.
	*/
	if (m_bDepthTextureSupported)
	{
		glGenTextures(1, &m_uiDepthTexture);
		glBindTexture(GL_TEXTURE_2D, m_uiDepthTexture);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, m_i32TexSize, m_i32TexSize, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_INT, NULL);
...
bool OGLES2Bloom::InitView()
{
	...
	if(strstr(pszExtensions, "GL_OES_depth_texture") == NULL)
	{
		PVRShellOutputDebug("GL_OES_depth_texture not supported, Bloom texture will be rendered incorrectly");
		m_bDepthTextureSupported = false;
	}
	else
	{
	...
	*/
	...
	for (unsigned int i=0; i < 2; i++)
	{
		...
		if (i==0 && m_bDepthTextureSupported)
...
bool OGLES2Bloom::RenderScene()
{
	...
	m_fModelAngleY += PVRT_PI / 200;
	m_fLightAngleY -= PVRT_PI / 300;

	PVRTMat4 mWorld = PVRTMat4::RotationY(m_fModelAngleY);
	PVRTMat4 mLight = PVRTMat4::RotationY(m_fLightAngleY);
	...
	if (m_bApplyBloom)
	{
		...
		*/
		...
		/* 
		   Alternatively you could make use of the built-in scissor testing facilities.
		...
		if (PVRShellGet(prefIsRotated) && PVRShellGet(prefFullScreen)) 
		{
			glScissor(50, 185, w - 115, h - 370);
		}
		else
		{
			glScissor(185, 50, w - 370, h - 115);
		}
		DrawAxisAlignedQuad(PVRTVec2(-1, -1), PVRTVec2(0, 0),
				            PVRTVec2(1, 1), PVRTVec2(1, 1));
	...
	m_Print3D.Print3D(2.0f, 90.0f, 1.5f, 0xFFFF8040, "%.2f fps", m_fFPS);
#endif

	// Displays the demo name using the tools. For a detailed explanation, see the training course IntroducingPVRTools	
	m_Print3D.DisplayDefaultTitle("Bloom", NULL, ePVRTPrint3DLogoIMG);
	m_Print3D.Flush();
...
******************************************************************************/
void OGLES2Bloom::UpdateFramerateCounter()
{
	long m_lPerf = PVRShellGetTime();
	++m_i32Frame;

	if((m_lPerf - m_lOldPerf) >= 1000l)
	{
		m_fFPS = m_i32Frame * 1000.0f / (float)(m_lPerf - m_lOldPerf);
		m_i32Frame = 0;
		m_lOldPerf = m_lPerf;
	}
}
