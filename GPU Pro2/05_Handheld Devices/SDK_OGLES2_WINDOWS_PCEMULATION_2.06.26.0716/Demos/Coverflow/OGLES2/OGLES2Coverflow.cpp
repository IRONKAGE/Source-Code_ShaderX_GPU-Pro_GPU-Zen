...
bool OGLES2Coverflow::LoadTextures(CPVRTString* const pErrorStr)
{
	for(int i = 0; i < g_i32CoverNo ; ++i)
	{
		...
		{
			CPVRTString sError = "Failed to load '" + CPVRTString(g_Covers[i].pTextureName) + "'.";
			PVRShellSet(prefExitMessage, sError.c_str());
			return false;
		}
		else
		{
	...
	*/
	...
	for(row = 0; row < 3; ++row)
	{
		for(col = 0; col < 3; ++col)
		{
			...
			if(row==1 && col == 1)
			{
				m_indicesOpaque[m_iNumOpaque++] = start+1;
				m_indicesOpaque[m_iNumOpaque++] = start;
				m_indicesOpaque[m_iNumOpaque++] = start+4;
				m_indicesOpaque[m_iNumOpaque++] = start+1;
				m_indicesOpaque[m_iNumOpaque++] = start+4;
				m_indicesOpaque[m_iNumOpaque++] = start+5;
			}
			else
			{
				m_indicesBlend[m_iNumBlend++] = start+1;
				m_indicesBlend[m_iNumBlend++] = start;
				m_indicesBlend[m_iNumBlend++] = start+4;
				m_indicesBlend[m_iNumBlend++] = start+1;
				m_indicesBlend[m_iNumBlend++] = start+4;
				m_indicesBlend[m_iNumBlend++] = start+5;
...
bool OGLES2Coverflow::RenderScene()
{
	...
	m_fLerp += (iDeltaTime*.0001f)*m_fCyclesPerSecond*m_fLerpDir;
...
void OGLES2Coverflow::DrawMesh()
{
	...
	glVertexAttribPointer(NORMAL_ARRAY, 3, GL_FLOAT, GL_FALSE, stride, (GLvoid*)offsetof(SVertex, n));
	glVertexAttribPointer(COLOR_ARRAY, 4, GL_FLOAT, GL_FALSE, stride, (GLvoid*)offsetof(SVertex, c));
	glVertexAttribPointer(TEXCOORD_ARRAY, 2, GL_FLOAT, GL_FALSE, stride, (GLvoid*)offsetof(SVertex, t));
