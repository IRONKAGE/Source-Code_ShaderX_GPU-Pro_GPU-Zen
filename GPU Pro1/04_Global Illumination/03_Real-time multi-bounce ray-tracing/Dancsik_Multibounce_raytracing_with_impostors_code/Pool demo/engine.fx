...
//*********************************************************************************	
//This shader stores diffuse objects in the environment distance impostor
//*********************************************************************************	
...
//*********************************************************************************	
bool intersectRefractor(float3 worldpos, float3 raydir, out float4 hitdata, out float3 hitpoint_refract, out float3 hitpoint_world)
{	
	float3 r = raydir;

	float3 refrpos = mul(float4(worldpos,1), sphereModelMatrixInverse).xyz;
	raydir = mul(float4(raydir,0), sphereModelMatrixInverse).xyz;
	
	//We calculate the intersection points with the bounding box.
	//If there are none, then neither will there be any intersections with the object.
	...
	//We can compute the intersection as the intersection of a new ray started along the original one,
	//from within the object, and in the opposing direction.			  
	float3 intersection = Hit(refrpos+dfar*raydir, -raydir, refractorMapSampler);
	hitdata = texCUBElod(refractorMapSampler, float4(intersection,0));
	
	float ratio = hitdata.a/length(intersection);
	if( abs( ratio - 1 ) > 0.03 ) return false;	 //if the ratio equals approximately to one, the intersection point is valid. 	
	
	hitpoint_refract = intersection * ratio;						//the intersection point in object space
	
	hitdata.rgb = mul(float4(normalize(hitdata.rgb),0), sphereModelMatrix).xyz;
	
	hitpoint_world = mul(float4(hitpoint_refract,1), sphereModelMatrix);		//the intersection point in world space
	
	if( dot(r, hitpoint_world-worldpos) < 0 ) return false;		//if the intersection point is invalid if it is behind the ray origin
...
void RefractorShading( inout float3 raydir, float3 normal, inout float3 refPos, inout float3 worldPos, float3 refractorPos, out float3 TBNPos, float refrIndex, out bool aboveWater, out float Fresnel )
{
	...
	float3 refracted_ray = refract(raydir, normal, refrIndex);		//1st refraction with the refractive object
	
	if( isTotalReflection(refracted_ray) )					//total reflection
	{
	...
	else				//2nd refraction								
	{
		...
		refracted_ray = mul(float4(refracted_ray,0), sphereModelMatrixInverse).xyz;
		
		float3 intersection =  Hit(refractorPos, refracted_ray, refractorMapSampler);
		float4 hitData = texCUBElod( refractorMapSampler, float4(intersection,0) );
		worldPos = mul(float4(intersection*hitData.a /length(intersection),1), sphereModelMatrix).xyz;
		refPos = worldPos - referencePointPosition;
		
		aboveWater = isAboveWater(worldPos, TBNPos);
		//the IOR depends on whether the exit point is under or above the water 
		if( aboveWater ) refrIndex = 1 / indexOfRefractionRefractor;
		else refrIndex = indexOfRefractionWater / indexOfRefractionRefractor; 
		
		float3 norm = normalize(-hitData.rgb);
		raydir = refract(refracted_ray, norm, refrIndex);
		
		if( isTotalReflection(raydir) )
		{
			raydir = reflect(refracted_ray, norm);
		}
		else 
			Fresnel *= ( 1 - FresnelFactorRefractor + ( 1 - FresnelFactorRefractor ) * pow( 1 - dot( norm, -refracted_ray ), 5) );
		
		raydir = mul(float4(refracted_ray,0), sphereModelMatrix);
	}	
